...
for ep_ix in range(n_epochs):
    ...
    for b_ix in range(n_train_steps):
        vid, _ = sess.run(train_dataset)
        if not b_ix%1000: verbosity = 2
        else: verbosity = 0
        model_256.fit(vid, vid, batch_size=100, epochs=1,
                         verbose=verbosity)
        
        model_128.fit(vid, vid, batch_size=100, epochs=1,
                         verbose=verbosity)
        model_64.fit(vid, vid, batch_size=100, epochs=1,
                         verbose=verbosity)
    # now test
    test_loss_list = []
    for b_test_ix in range(n_test_steps):
        vid_test, _ = sess.run(test_dataset)
        test_loss = model_256.test_on_batch(vid_test, vid_test)
        test_loss_list.append(test_loss)
        test_loss = model_128.test_on_batch(vid_test,vid_test)
        test_loss_list.append(test_loss)
        test_loss = model_64.test_on_batch(vid_test, vid_test)
        test_loss_list.append(test_loss)
    ...
    if not ep_ix%10: model_256.save('v2v_256_ep_%d.h5' % ep_ix )
    if not ep_ix%10: model_128.save('v2v_128_ep_%d.h5' % ep_ix )
    if not ep_ix%10: model_64.save('v2v_64_ep_%d.h5' % ep_ix )
